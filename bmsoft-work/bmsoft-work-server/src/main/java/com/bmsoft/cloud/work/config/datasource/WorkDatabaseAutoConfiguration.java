 ...
 * <p>
 * NONE("非租户模式"): 不存在租户的概念 COLUMN("字段模式"): 在sql中拼接 tenant_code 字段
 * SCHEMA("独立schema模式"): 在sql中拼接 数据库 schema
 * <p>
 * COLUMN和SCHEMA模式的实现 参考下面的 @see 中的3个类
 *
 * @author bmsoft
 * @date 2020-07-24 断点查看原理：👇👇👇
...
@MapperScan(basePackages = {
		"com.bmsoft.cloud", }, annotationClass = Repository.class, sqlSessionFactoryRef = WorkDatabaseAutoConfiguration.DATABASE_PREFIX
				+ "SqlSessionFactory")
@EnableConfigurationProperties({ MybatisPlusProperties.class })
@ConditionalOnExpression("!'DATASOURCE'.equals('${bmsoft.database.multiTenantType}')")
public class WorkDatabaseAutoConfiguration extends BaseDatabaseConfiguration {
	/**
	 * 每个数据源配置不同即可
	 */
	...
	public WorkDatabaseAutoConfiguration(MybatisPlusProperties properties, DatabaseProperties databaseProperties,
		...
		super(properties, databaseProperties, interceptorsProvider, typeHandlersProvider, languageDriversProvider,
		...
		log.debug("检测到 bmsoft.database.multiTenantType!=DATASOURCE，加载了 WorkDatabaseAutoConfiguration");
	}
