...
使用 bmsoft 帐号安装。某些文件夹没有权限的， 需要sudo授权
...
* 配置tracker所需的base_path:
    + 192.168.65.129机子上创建 /home/bmsoft/fastdfs/tracker/
* 配置storage所需的日志目录:
    + 192.168.65.130机子上创建/home/bmsoft/fastdfs/storage/logs
* 配置storage所需的存储文件目录:
    + 192.168.65.130机子上创建/home/bmsoft/fastdfs/storage/data
...
```
 ...
 sudo chown -R bmsoft:bmsoft /etc/fdfs/
```
...
base_path=/home/bmsoft/fastdfs/storage
store_path_count=1   #存储路径个数，需要和store_path个数匹配
store_path0=/home/bmsoft/fastdfs/storage
tracker_server=123.249.76.119:22122 #tracker服务器的IP地址和端口号 这里若有多台，则配置多个  不能配置127.0.0.1
http.server_port=7888

vim tracker.conf
base_path=/home/bmsoft/fastdfs/tracker
...
* b. 通过以下命令查看tracker的启动日志，看是否有错误: `tail -100f  /home/bmsoft/fastdfs/tracker/logs/trackerd.log`
* c. 通过以下命令查看storage的启动日志，看是否有错误: `tail -100f  /home/bmsoft/fastdfs/storage/logs/storaged.log`
...
```
base_path=/home/bmsoft/fastdfs/tracker #tracker服务器文件路径
...
```
执行：`/usr/bin/fdfs_upload_file  /etc/fdfs/client.conf  /home/bmsoft/tools/fastdfs/logo.jpg`
...
./configure --user=bmsoft --group=bmsoft --prefix=/usr/local/nginx --add-module=/usr/local/src/fastdfs-nginx-module/src --add-module=/usr/local/src/ngx_cache_purge-2.1
./configure --user=bmsoft --group=bmsoft --prefix=/usr/local/nginx --add-module=/usr/local/src/ngx_cache_purge-2.1
make
sudo make install
...
server {
        listen 6080;
        #server_name www.bmsoft.com;  # Here to modify the domain name by 
        ...
        location ~/group1/M00{
                ...
                root    /home/bmsoft/fastdfs/storage/data;
                ngx_fastdfs_module;
...
store_path0=/home/bmsoft/fastdfs/storage # 存储路径
group_count = 1 #设置组的个数，事实上这次只使用了group1

```
建立软连接:
`ln  -s  /home/bmsoft/fastdfs/storage/data  /home/bmsoft/fastdfs/storage/M00`
...
1. 打开 /etc/fdfs 文件夹，编辑 client.conf 文件，编辑内容如下:
  a. base_path=/home/bmsoft/fastdfs/tracker      #存放路径
...
2. 模拟上传文件，执行如下命令: /usr/bin/fdfs_upload_file  /etc/fdfs/client.conf  /home/bmsoft/tools/fastdfs/logo.jpg
...
port=11411
base_path=/home/bmsoft/fastdht （该目录必须是已经存在的）
cache_size = 32MB
...
| /usr/bin/fdfs_upload_file  /etc/fdfs/client.conf  /home/bmsoft/tools/fastdfs/logo.jpg | 测试上传                                     |
