...
- 修改 初始密码账号、路径
```
-e RABBITMQ_DEFAULT_USER=bmsoft -e RABBITMQ_DEFAULT_PASS=bmsoft \       # 账号密码    
...
```
- 将 bmsoft.conf 文件中的所有 localhost 替换成宿主机的内网ip。 如： 192.168.1.1
- 启动
```
...
cd bmsoft-backend/bmsoft-authority/bmsoft-authority-server
docker build --build-arg JAR_FILE=target/bmsoft-authority-server.jar -t bmsoft-authority-server:v1 .

cd bmsoft-backend/bmsoft-msgs/bmsoft-msgs-server
docker build -t bmsoft-msgs-server:v1 .

cd bmsoft-backend/bmsoft-file/bmsoft-file-server
docker build -t bmsoft-file-server:v1 .

cd bmsoft-backend/bmsoft-gateway/bmsoft-zuul-server
docker build -t bmsoft-zuul-server:v1 .

cd bmsoft-backend/bmsoft-jobs/bmsoft-jobs-server
docker build -t bmsoft-jobs-server:v1 .
```

## 7，使用docker启动项目
```
1, 确保： nacos mysql redis 全部启动， 并且nacos 里面的redis mysql 等ip改成自己的ip （切记，使用docker时，所有的IP都不能为127.0.0.1）
2, 在nacos 里面新建  bmsoft-authority-server-docker.yml ,并将所有bmsoft-authority-server-dev.yml 里面的配置复制过去，修改docker环境和dev环境的差异部分部分。

docker run -idt  --name bmsoft-authority-server --restart=always \
        ...
        -p 8764:8764 \
        -v /data/:/data \
        bmsoft-authority-server:v1 

docker run -idt  --name bmsoft-zuul-server --restart=always \
        -e NACOS_IP=192.168.1.34 \
        -e NACOS_PORT=8848 \
        -e LOCAL_IP=192.168.1.34 \
        -e NACOS_ID=b16f7baf-56e7-4f4e-a26c-425ee0668016 \
        -p 8760:8760 \
        -v /data/:/data \
        bmsoft-zuul-server:v1 

docker run -idt  --name bmsoft-msgs-server --restart=always \
        -e NACOS_IP=192.168.1.34 \
        -e NACOS_PORT=8848 \
        -e LOCAL_IP=192.168.1.34 \
        -e NACOS_ID=b16f7baf-56e7-4f4e-a26c-425ee0668016 \
        -p 8768:8768 \
        -v /data/:/data \
        bmsoft-msgs-server:v1 

docker run -idt  --name bmsoft-file-server --restart=always \
        -e NACOS_IP=192.168.1.34 \
        -e NACOS_PORT=8848 \
        -e LOCAL_IP=192.168.1.34 \
        -e NACOS_ID=b16f7baf-56e7-4f4e-a26c-425ee0668016 \
        -p 8765:8765 \
        -v /data/:/data \
        bmsoft-file-server:v1 

docker run -idt  --name bmsoft-jobs-server --restart=always \
        -e MYSQL_IP=192.168.1.34 \
        -p 8767:8767 \
        -v /data/:/data \
        bmsoft-jobs-server:v1 
