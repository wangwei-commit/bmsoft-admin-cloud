## 常见问题：
0. 更多常见问题请看： https://www.kancloud.cn/bmsoft/bmsoft-admin-cloud/1284774
...
6. 对于window服务器， 启动bmsoft-jobs-server报找不到表的错， 但数据库中确实有表，只是表名全是小写？    
   ```
   答：   1） SHOW VARIABLES LIKE '%case%'; 查看 lower_case_table_names 是否=2  
      ...
      ```   
6.1 对于Linux服务器， 启动bmsoft-jobs-server报找不到表的错， 但数据库中确实有表，只是表名全是小写？    
   ```
...
7. nacos里面配置文件10多个，bmsoft-xx-server/src/main/resources/*.yml 下面也有 bootstrap.yml、bootstrap-dev.yml、application.yml 
    ```
   答： 建议先网上搜索了解清楚 SpringBoot 项目的 bootstrap.yml、application.yml 的加载优先级, 以及多环境配置
    
    本项目中的，这几个配置文件中出现了相同的key，但不同的value时，
    value读取顺序时： bmsoft-authority-server-dev.yml(nacos) > bmsoft-authority-server.yml(nacos) > common.yml(nacos) > bootstrap-dev.yml(resources) > bootstrap.yml(resources)
    比如：bmsoft.port.nginx: ${server.port}  bmsoft.port.nginx: ${NGINX_PORT:80}
...
8. 启动时， 报连不上数据库的错？ `Failed to configure a DataSource: 'url' attribute is not specified and no embedded datasource could be configured.`
     ...
     2，检查 nacos 的`配置列表` - 切换到命名空间 `bmsoft`  -  查看 `mysql.yml` 和 `bmsoft-**-server-dev.yml` 里面关于数据库的配置，是否正确.
     3，确定一下，是否在在 IDEA 中执行过 `clean insatll ` 编译命令！ （这一步很关键！）
     4，打开`bmsoft-authority/bmsoft-authority-server/target/classes/bootstrap.yml` 文件，检查一下 `@nacos.ip@` 、`@nacos.port@` 、`@nacos.namespace@`  等 `@xx@` 相关的表达式是否替换成具体的值了。 
     5，确认一下nacos中，所有的配置文件，`点击编辑`-`配置格式` 修改为YAML 类型。（nacos的bug，导致导入的配置会变成TEXT类型）
     6，确认一下nacos中，**bmsoft命名空间下**，mysql.yml 和 bmsoft-authority-server-dev.yml中，mysql相关的 信息是否配对。
    ```
9. 启动时，报dozer 的一系列错误？
    ```
...
java -jar -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=5005 bmsoft-authority-server.jar
...
- 在jenkins服务器，将私钥复制到 应用服务器
    ssh-copy-id -i ~/.ssh/id_rsa.pub -p 53535 bmsoft@192.168.30.1
...
doc/sql/bmsoft_authority_dev_data.sql
...
third-party/nacos/target/nacos-server.jar
doc/sql/bmsoft_authority_dev.sql
bmsoft-backend/bmsoft-gateway/bmsoft-gateway-server/src/main/resources/META-INF/resources/webjars/springfox-swagger-ui/swagger-ui.js
bmsoft-frontend/bmsoft-manage-center/src/main/resources/static/fonts/fzltxh.ttf
bmsoft-frontend/bmsoft-manage-center/src/main/resources/static/libs/eonasdan-bootstrap-datetimepicker/src/nuget/NuGet.exe
...
git filter-branch -f --tree-filter 'rm -f bmsoft-backend/bmsoft-gateway/bmsoft-gateway-server/src/main/resources/META-INF/**' --tag-name-filter cat -- --all
git filter-branch -f --tree-filter 'rm -f bmsoft-backend/bmsoft-gateway/bmsoft-zuul-server/src/main/resources/META-INF/**' --tag-name-filter cat -- --all
...
git filter-branch -f --tree-filter 'rm -f bmsoft-frontend/**' --tag-name-filter cat -- --all
